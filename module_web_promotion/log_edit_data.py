...
def log_event(connection_params, event_type, employee_code, employee_name, old_value=None, new_value=None, description=""):
    try:
        # สร้างข้อความ log
        timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        log_message = f"{timestamp} - Event: {event_type.upper()}"
        ...
        if old_value is not None and new_value is not None:
            ...
            log_message += f" - Removed Value: {old_value}"
        elif new_value is not None:
            log_message += f" - Added Value: {new_value}"
