            ...
            try:
				...
				CTE AS (
				    SELECT
				        ...
				        sml_emi.move_id,
				        sml_emi.lot_name,
				        ROW_NUMBER() OVER (PARTITION BY sml_emi.lot_name ORDER BY sml_emi.id) AS rn_po
				    FROM
				        stock_move_line sml_emi
				    WHERE
				        sml_emi.lot_name IS NOT NULL
				),
                PosOrderLine as (
                    SELECT 
                        pos_line.order_id,pos_line.product_id,pos_line.id,pos_line.qty,pos_line.price_unit,pos_line.price_subtotal,pos_line.price_subtotal_incl,
                ...
                select
                   ...
                   sol.price_unit as unit_price,
                    ...
                    pp.id as product_id,
                    null as pos_id,
                    po_pro.date_order as po_date_order,
					rp_po.name as partner_name
                from
						sale_order so
                    ...
                    left join CTE AS sml_po ON sml_po.lot_name = s_pro_l.name AND sml_po.rn_po = 1
					left join stock_move sm_po on sm_po.id = sml_po.move_id
                    left join purchase_order_line pol_pro on pol_pro.id = sm_po.purchase_line_id 
					left join purchase_order po_pro on po_pro.id = pol_pro.order_id 
					left join res_partner rp_po on rp_po.id = po_pro.partner_id 
                ...
                data = [{
                            ...
                            'pos_id':item.pos_id,
                            'po_date_order':item.po_date_order.strftime('%d/%m/%Y') if item.po_date_order is not None else '-' ,
