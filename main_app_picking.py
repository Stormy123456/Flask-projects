...
from config_db import connection_string, db_test
import pyodbc 
...
def login():
    if request.method == 'POST':
        ...
        if auth.login(employee_code, password):
            ...
            redirect_url = url_for('web_promotion_cost')  # URL ที่ต้องการเปลี่ยนหน้าไป
            return render_template('web_promotion/login.html', success_message=success_message, redirect_url=redirect_url)
        else:
            # ถ้าเข้าสู่ระบบไม่สำเร็จ
            error_message = "เข้าสู่ระบบไม่สำเร็จ โปรดตรวจสอบรหัสพนักงานหรือรหัสผ่านของท่าน."
            return render_template('web_promotion/login.html', error_message=error_message)
    return render_template('web_promotion/login.html')
...
#         # if auth.login(employee_code, password):
#         #     return redirect(url_for('web_promotion_price_set'))
#         # else:
...
#             # ถ้าเข้าสู่ระบบสำเร็จ
#             success_message = "เข้าสู่ระบบสำเร็จ."
#             redirect_url = url_for('web_promotion_price_set')  # URL ที่ต้องการเปลี่ยนหน้าไป
#             return render_template('login.html', success_message=success_message, redirect_url=redirect_url)
#         else:
#             # ถ้าเข้าสู่ระบบไม่สำเร็จ
#             error_message = "เข้าสู่ระบบไม่สำเร็จ โปรดตรวจสอบรหัสพนักงานหรือรหัสผ่านของท่าน."
...
def web_promotion_price_set():
    data = []  # กำหนดค่าเริ่มต้นเป็นลิสต์ว่าง
    cost_data = []  # กำหนดค่าเริ่มต้นเป็นลิสต์ว่าง 
    connection = None
...
>>>>>>> BIG
                    ...
                    # Query สำหรับดึงข้อมูลจาก cost_and_status
                    cost_data_query = """select 
                                    *
                                from
                                    cost_and_status cs
...
>>>>>>> BIG
                ...
                return render_template('web_promotion/cost_and_status.html', data=data, odoo_product=odoo_product, request=request)
            else:
                flash('Access denied: insufficient permissions.')
                return redirect(url_for('login'))
        else:
    ...
    try:
        ...
        if 'employee_code' in session:
            if int(session.get('type_status')) == 3:
                ...
                return render_template('web_promotion/set_premium.html', data=data, request=request)
            else:
                flash('Access denied: insufficient permissions.')
                return redirect(url_for('login'))
        else:
